...
- `친구비` 는 **정산액이다.** 누가 낼 몫인지는 `방향` 이 말하고 크기는 `금액(원)` 이 말한다.
  방향을 그대로 옮겨 쓴다. 방향이 어느 쪽이든 **나쁜 관계라는 뜻이 아니다.**
  내가 낼 몫이라는 것은 "상대가 더 기여했다"는 뜻이다.
  관계가 좋은지 나쁜지는 친밀도와 손절위험도가 말한다.
- **`음수`, `양수`, `마이너스`, `플러스` 같은 말을 쓰지 않는다.**
  사용자는 화면에서 부호를 본 적이 없다. 방향과 금액만 본다.
  글에만 부호가 나오면 화면에 없는 개념을 설명하는 셈이다.
- **친구비는 실제로 오간 돈이 아니다.** 기여 격차를 금액에 빗댄 것이다.
  "정산하세요", "갚으세요", "다음에 계산하세요" 같은 조언을 쓰지 않는다.
  실제로 빌려주거나 빌린 일은 `moneySignals` 가 따로 말한다. 둘을 섞지 않는다.
