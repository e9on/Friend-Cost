# Cloud Run 배포 (채택하지 않음)

> **2026-08-28: 국내 VPS 로 정했다. `vps.md` 를 본다.**
>
> 이 문서는 검토 과정을 남기려고 둔다. 아래 계산은 그대로 쓰면 안 된다.

> **이 문서의 전제가 바뀌었다.**
>
> 아래 계산은 OCR 을 **Google Vision API** 로 부르던 시절 것이다. 지금은
> RapidOCR 이 컨테이너 안에서 돈다. 그래서 두 가지가 달라졌다.
>
> | | 옛 전제 | 지금 |
> | --- | --- | --- |
> | 메모리 | 512Mi, 동시 8건 | **분석 1건에 754Mi** (실측) |
> | 스케일-투-제로 | 이점이 컸다 | 콜드 스타트마다 ONNX 모델을 다시 올린다 |
> | 비용 구조 | 건당 과금(OCR API) | **고정비**(상시 실행) |
>
> `tools/cost_model.py` 기준으로 저가 VPS 월 9,800원, Cloud Run 상시
> 27,594원이다. **RapidOCR 을 쓰는 한 상시 실행이 필요하므로 VPS 가 3배
> 싸다.** 아래 Cloud Run 절차는 참고로 남겨두되, 512Mi/동시성 8 은 그대로
> 쓰면 안 된다.
>
> 리눅스 컨테이너에서 메모리를 다시 재고 동시성을 정하는 것이 먼저다.
> 위 754Mi 는 Windows 측정치다.
