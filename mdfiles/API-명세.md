...
`jobId`는 UUID v4이며 **이것이 사실상의 접근 토큰이다.** 별도 인증이 없으므로 `jobId`를 아는 클라이언트만 결과를 조회할 수 있다.

애플리케이션 로그와 오류 메시지에 `jobId`를 넣지 않는다. 테스트가 이를 강제한다.

다만 **`jobId`는 URL 경로에 있으므로 배포 플랫폼의 접근 로그에는 남는다.** 우리가 통제할 수 없는 영역이다. TTL 5분이 노출 창을 좁히지만 없애지는 못한다. 한계는 `운영-보안-법적고지-명세.md` 6.3에 적어두었다.
...
- 이름은 `데이터-계약-명세.md` 12-1 의 목록만 받는다. 그 밖은 **400**
- 응답 본문이 없다. 화면은 결과를 기다리지 않는다
- 실패해도 화면은 그대로 진행한다. 집계가 사용자 흐름을 막아서는 안 된다
- 요청 제한은 폴링과 같은 창을 쓴다

**인증이 없으므로 아무나 호출할 수 있다.** 숫자가 부풀 수 있다는 뜻이다. 이 값을 정확한 통계로 쓰지 않고 **경향을 보는 데만** 쓴다.
...
고정형을 쓰는 이유는 최대 보관 시간을 5분으로 확실히 잠그기 위해서다. 조회할 때마다 연장하면 새로고침을 반복하는 것만으로 무기한 보관이 되어 기준 명세 5장의 영구 저장 금지 원칙이 흔들린다.
...
클라이언트에서 결과 이미지 렌더링 및 저장
       ↓
DELETE /v1/analyses/{jobId}  또는  TTL 5분 만료
```
...
| 버전 | 날짜 | 내용 |
| --- | --- | --- |
| v1.4 | 2026-08-27 | TTL 을 5분으로. 사용 이벤트 엔드포인트(7-1), `SERVICE_DAILY_LIMIT`·`SERVICE_ENDED` 오류 추가. `confidence` 를 응답에서 제거 |
| v1.3 | 2026-08-26 | 최대 화소 수 상한 추가. 용량만 검사하면 33바이트 파일이 9억 화소를 선언해도 통과했다 |
| v1.2-1 | 2026-08-26 | `friendFee` 가 음수를 가질 수 있다. 정의는 `데이터-계약-명세.md` |
