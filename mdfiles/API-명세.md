...
해상도는 이미지 헤더에서 직접 읽는다. 이미지 라이브러리를 쓰지 않는 이유는 콜드 스타트가 중요한 배포 형태이기 때문이다. 320px보다 작으면 글자를 읽을 수 없으므로 OCR을 돌리기 전에 거절한다. 형식·해상도 위반은 모두 `IMAGE_FORMAT_UNSUPPORTED`로 응답한다.
