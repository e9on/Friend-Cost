...
| 월 분석 | Vision + Groq | CLOVA + Groq | 자체 호스팅 + Groq |
| ---: | ---: | ---: | ---: |
| 100건 | 0원 | 0원 | 9,800원 |
| 500건 | 0원 | 15,000원 | 9,800원 |
| 1,000건 | **8,400원** | 40,000원 | 9,800원 |
| 5,000건 | 54,400원 | 244,000원 | **13,800원** |
| 20,000건 | 208,000원 | 990,000원 | **45,000원** |

> 분석당 이미지 5장 기준. `backend/tools/cost_model.py` 로 재계산할 수 있다.

**LLM이 아니라 OCR이 비용을 지배한다.** Groq 무료 티어는 월 15,000건까지 0원인데, OCR은 건당 과금이라 선형으로 오른다. 월 20,000건에서 CLOVA는 99만 원, Vision도 21만 원이다.

이전 보고서에 적은 "월 2,000건을 넘어가면 자체 호스팅을 재검토한다"는 **틀린 숫자였다.** 실제 전환점은 **월 약 1,141건**이다.
...
| 후보 | 좌표 | 한국어 | 100건/월 | 5,000건/월 | 스케일-투-제로 |
| --- | --- | --- | ---: | ---: | --- |
| **Google Cloud Vision** | 제공 | 상위권 | 0원 | 50,400원 | 유지 |
| **Naver CLOVA OCR** | 제공 | 최상위(97%+) | 0원 | 240,000원 | 유지 |
| Upstage Document OCR | 제공 | 국내 특화 | 페이지당 $0.01 | 약 700,000원 | 유지 |
| **RapidOCR (자체)** | 제공 | 모델 교체 필요 | 9,800원 | 9,800원 | **포기** |
| EasyOCR (자체) | 제공 | 기본 지원 | 9,800원+ | 9,800원+ | **포기** |
| PaddleOCR (자체) | 제공 | 별도 모델 | 9,800원+ | 9,800원+ | **포기** |

Upstage는 페이지당 $0.01로 Vision의 약 7배다. 국내 문서 특화가 강점이지만 우리는 정형 문서가 아니라 채팅 캡처를 다루므로 그 강점이 살지 않는다. **후보에서 제외한다.**

### 2.3 자체 호스팅의 진짜 비용

표의 "9,800원"은 소형 상시 인스턴스 기준이며 **하한선이다.** 실제로는 세 가지가 더 붙는다.
...
즉 **전환점 1,141건은 비용만 본 숫자**이고, 운영 부담을 넣으면 더 뒤로 밀린다.
...
| 성장 | 1,000~5,000건 | Vision 유지 또는 자체 호스팅 검토 | 5,000건에서 5만 원. 감당 가능하면 운영 단순함이 낫다 |
| 확대 | 5,000건~ | **자체 호스팅** | 비용 차이가 4배를 넘는다 |
...
- **자체 호스팅 시 컨테이너 크기와 콜드 스타트 시간.** 동시성 예산에 직접 영향을 준다.
...
| --- | --- | --- |
| 비용의 지배 요인 | LLM | **OCR** |
| 자체 호스팅 전환점 | 월 2,000건 | **월 약 1,141건** (비용만 기준) |
| CLOVA 위치 | 2순위 대안 | **후보에서 내림** (Vision의 5배) |
| 자체 호스팅 시점 | "볼륨이 커진 뒤" | **지금 평가 시작** |
