...
**"자체 호스팅"이나 "로컬 OCR"이라는 말은 오해를 부른다.** 정확한 표현은 **컨테이너 내장 OCR**이다.

```text
[사용자 휴대폰]  ── 캡처 업로드 ──▶  [Cloud Run 컨테이너]
                                        ├ FastAPI
                                        ├ OCR 모델 15MB   ← 여기서 돈다
                                        ├ Parser
                                        └ LLM 호출 ──▶ [Groq API]
...
지금 설계는 OCR 자리에서 Google Vision API를 부른다. 바뀌는 것은 그 한 군데뿐이고, **배포처는 그대로 Cloud Run이다.** 개발자 PC는 어느 모델이 쓸 만한지 재보는 평가 환경일 뿐 서비스와 무관하다.
...
5장의 1단계를 실행했다. `render_kakao.py` 로 합성 카카오톡 캡처 5장(메시지 70개)을 그리고 `benchmark_ocr.py` 로 채점했다.

### 6.1 모델별 성적

| 모델 | 문장 완전 일치 | 문자 오류율(CER) | 화자 판별 | 장당 처리 |
| --- | ---: | ---: | ---: | ---: |
| 기본(중국어·영어) | 0.0% | — | 100% | 0.87초 |
| `korean_mobile_v2.0` (2021) | 4.3% | 54.0% | 100% | 0.51초 |
| **`korean_PP-OCRv4_rec_mobile`** | **58.6%** | **31.7%** | **100%** | **1.61초** |

**좌표 기반 화자 판별은 세 경우 모두 100%다.** 우리 설계에서 가장 중요한 지표가 인식 품질과 무관하다는 뜻이다. `OCR-Parser-명세.md` 4장이 여백 비교를 쓰기로 한 판단이 여기서 확인됐다.

`PP-OCRv5` 를 쓰지 않은 것은 **v5 에 한국어 전용 모델이 없기 때문이다.** v5 는 중국어·영어·일본어 혼합을 다루고, 한국어 전용은 v4 가 마지막이다.

### 6.2 기본 모델의 위험

기본 모델로 돌리면 **한글을 한 글자도 읽지 못하는데 오류가 나지 않는다.** 좌표는 잘 잡으므로 화자 판별 100%, 처리 시간 정상, 예외 없음이다. 결과만 비어 있다.

이 상태로 배포하면 LLM 은 빈 대화를 받아 그럴듯한 리포트를 쓴다. 그래서 `app/infrastructure/ocr/rapid.py` 가 한국어 모델을 **기본값으로 못 박는다.**

### 6.3 함께 드러난 결함

글자가 읽히기 시작하자 Parser 가 `GROUP_CHAT_DETECTED` 로 거부했다. 단체방 감지가 폰트 높이에 기대고 있었는데, "나 지금 회사야" 같은 평범한 메시지가 발신자 이름으로 오인됐다. 실측으로 높이비 분포가 완전히 겹치는 것을 확인하고 반복 횟수 기준으로 바꿨다. 자세한 내용은 `OCR-Parser-명세.md` 7장.

**두 결함 모두 스텁으로는 드러나지 않는다.** 스텁은 항상 올바른 텍스트를 돌려주기 때문이다.
...
CER 31.7% 는 낮지 않다. 못 읽은 것은 "헐 대박", "ㅇㅇ" 처럼 **아주 짧은 말**과 일부 긴 문장에 몰려 있다. 자모(`ㅋㅋ`, `ㅠㅠ`)가 특별히 나쁘지는 않았다(자모 포함 25.0% vs 일반 문장 32.4%).

검출기를 바꿔봤지만 나아지지 않았다.

| 조합 | CER |
| --- | ---: |
| 기본 검출기(PP-OCRv6 small) | **31.7%** |
| PP-OCRv4 mobile 검출기 | 34.1% |
| PP-OCRv4 server 검출기 | 33.3% (6.26초/장) |

**여기서 더 손대지 않는다.** 합성 캡처는 우리가 그린 것이라, 더 맞추면 실제 캡처가 아니라 렌더러에 과적합된다.

이 결과의 한계는 분명하다.

- 합성 캡처에는 압축 잡음, 프로필 사진, 읽음 표시, 다양한 글꼴이 없다. **여기서 나온 점수는 하한이 아니라 상한에 가깝다.**
- 실제 캡처로 다시 재야 한다. `운영-보안-법적고지-명세.md` 8장 점검표.
- Google Vision 과 나란히 재지 못했다. GCP 계정이 필요하다.

## 7. 미확인 항목

- ~~**한국어 모델을 얹은 RapidOCR의 실제 인식률.**~~ 2026-08-26 실측으로 해소했다. 6장 참조.
...
| 버전 | 날짜 | 내용 |
| --- | --- | --- |
| v1.2 | 2026-08-26 | **실측 완료.** 6장 신설. `korean_PP-OCRv4_rec_mobile` 채택(CER 31.7%, 화자 판별 100%). 기본 모델이 한글을 못 읽으면서도 오류를 내지 않는 위험, 단체방 감지 결함을 함께 기록 |
